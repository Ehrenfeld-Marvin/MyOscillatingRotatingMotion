...
{
    defineTypeNameAndDebug(Myforces, 0);
    addToRunTimeSelectionTable(functionObject, Myforces, dictionary);
}
}
...
Foam::word Foam::functionObjects::Myforces::fieldName(const word& name) const
{
    return this->name() + ":" + name;
}


void Foam::functionObjects::Myforces::createFiles()
{
	...
	Force_Array_Ptr = createFile("Force_Array");
        ...
        if (nBin_ > 1)
        {
            ...
            writeBinHeader("MyForce", MyforceBinFilePtr_());
...
void Foam::functionObjects::Myforces::setCoordinateSystem
(
...
)
{
    ...
    else
    {
        ...
        // but enMyforce use of a cartesian system if not found.
...
void Foam::functionObjects::Myforces::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::Myforces::initialiseBins()
{
    if (nBin_ > 1)
    {
    ...
    // Allocate storage for Myforces and moments
    forAll(Myforce_, i)
    {
        Myforce_[i].setSize(nBin_, vector::zero);
        moment_[i].setSize(nBin_, vector::zero);
    }
}


void Foam::functionObjects::Myforces::resetFields()
{
    Myforce_[0] = Zero;
    Myforce_[1] = Zero;
    Myforce_[2] = Zero;
    ...
    if (writeFields_)
    {
        volVectorField& Myforce =
            lookupObjectRef<volVectorField>(fieldName("Myforce"));

        Myforce == dimensionedVector(Myforce.dimensions(), Zero);
...
Foam::functionObjects::Myforces::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::Myforces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::Myforces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::Myforces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::Myforces::applyBins
(
...
)
{
    ...
    else
    {
        ...
        forAll(dd, i)
        {
            label bini = min(max(floor(dd[i]/binDx_), 0), Myforce_[0].size() - 1);

            Myforce_[0][bini] += fN[i];
            Myforce_[1][bini] += fT[i];
            Myforce_[2][bini] += fP[i];
...
void Foam::functionObjects::Myforces::addToFields
(
    const label patchi,
    const vectorField& Md,
    const vectorField& fN,
...
)
{
    ...
    auto& Myforce = lookupObjectRef<volVectorField>(fieldName("Myforce"));
    vectorField& pf = Myforce.boundaryFieldRef()[patchi];
    pf += fN + fT + fP;
...
)
{
    ...
    forAll(cellIDs, i)
    {
        ...
        Myforce[celli] += fN[i] + fT[i] + fP[i];
        moment[celli] = Md[i]^Myforce[celli];
    }
}
...
void Foam::functionObjects::Myforces::writeBinnedMyForceMoment
(
...
void Foam::functionObjects::Myforces::writeBins()
{
    List<Field<vector>> lf(3);
    List<Field<vector>> lm(3);
    lf[0] = coordSys_.localVector(Myforce_[0]);
    lf[1] = coordSys_.localVector(Myforce_[1]);
    lf[2] = coordSys_.localVector(Myforce_[2]);
    lm[0] = coordSys_.localVector(moment_[0]);
    lm[1] = coordSys_.localVector(moment_[1]);
    lm[2] = coordSys_.localVector(moment_[2]);

    writeBinnedMyForceMoment(lf, MyforceBinFilePtr_);
    writeBinnedMyForceMoment(lm, momentBinFilePtr_);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::functionObjects::Myforces::Myforces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::Myforces::read(const dictionary& dict)
{
    ...
    directMyForceDensity_ = dict.getOrDefault("directMyForceDensity", false);
    ...
    if (writeFields_)
    {
        ...
        mesh_.objectRegistry::store(MyforcePtr);
...
void Foam::functionObjects::Myforces::calcMyForcesMoment()
{
    initialise();
    ...
    if (directMyForceDensity_)
    {
        ...
        for (const label patchi : patchSet_)
        {
            ...
            // Normal Myforce = surfaceUnitNormal*(surfaceNormal & MyforceDensity)
            vectorField fN
            (
                Sfb[patchi]/sA
               *(
            ...
            // Tangential Myforce (total Myforce minus normal fN)
    ...
    Pstream::listCombineGather(Myforce_, plusEqOp<vectorField>());
    Pstream::listCombineGather(moment_, plusEqOp<vectorField>());
    Pstream::listCombineScatter(Myforce_);
    Pstream::listCombineScatter(moment_);
}


Foam::vector Foam::functionObjects::Myforces::MyforceEff() const
{
    return sum(Myforce_[0]) + sum(Myforce_[1]) + sum(Myforce_[2]);
}


Foam::vector Foam::functionObjects::Myforces::momentEff() const
{
    return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool Foam::functionObjects::Myforces::execute()
{
    calcMyForcesMoment();
    ...
    setResult("normalMyForce", sum(Myforce_[0]));
    setResult("tangentialMyForce", sum(Myforce_[1]));
    setResult("porousMyForce", sum(Myforce_[2]));
...
bool Foam::functionObjects::Myforces::write()
{
    if (writeFields_)
    {
        lookupObject<volVectorField>(fieldName("Myforce")).write();
