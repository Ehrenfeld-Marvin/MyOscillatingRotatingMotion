...
#include "solidBodyMotionSolverTrim.H"
...
namespace Foam
{
    defineTypeNameAndDebug(solidBodyMotionSolverTrim, 0);
    addToRunTimeSelectionTable
    (
        motionSolver,
        solidBodyMotionSolverTrim,
        dictionary
    );
}
...
Foam::solidBodyMotionSolverTrim::solidBodyMotionSolverTrim
(
    const polyMesh& mesh,
...
:
    ...
    SBMFPtr_(solidBodyMotionFunctionTrim::New(coeffDict(), mesh.time())),
    pointIDs_(),
    moveAllCells_(false)
{
    word cellZoneName =
...
Foam::tmp<Foam::pointField> Foam::solidBodyMotionSolverTrim::curPoints() const
{
    if (moveAllCells_)
    {
